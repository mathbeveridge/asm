...
import aztec.ian_comb as ian_comb



# one problem for n=4 and 10 for n=5
############

# def untangle_v1(triangle, row_idx, col_max_idx, debug=False):
...
#                     raise ValueError('Above to level at a diagonal TBD')
#             elif diff > 0:
...
#                     big_row[k] = big_row[k] + diff
#                     small_row[k] = small_row[k] - diff
#
#                 level_start_idx = size
#
# #            crossed_delta = max(diff, crossed_delta)
...
#
#         elif state == LEVEL:
#             # is this  a shared diagonal?
#             if small_row[level_start_idx] > small_row[idx]:
#
#                 end_idx = idx + 1
#
#                 while end_idx < col_max_idx and small_row[end_idx]  == big_row[end_idx]:
#                     end_idx += 1
#
#                 # need to create drop!
# #                if idx == col_max_idx - 1 and big_row[idx+1] == big_row[idx]:
#                 #if end_idx == col_max_idx and big_row[end_idx] == big_row[end_idx -1]:
#                 while big_row[end_idx-1] == big_row[end_idx]:
#                     end_idx = end_idx - 1
#                 #else:
#                 #    end_idx = idx+1
...
#                 state = ABOVE
#                 level_start_idx = size
#             elif idx == col_max_idx-1 and small_row[idx] > big_row[idx+1]:
...
# This one doesn't deal with the [2,1,1],[2,1] problem or thee
# [3,2,2,2],[3,2,2] problem
# def untangle_v2(triangle, row_idx, col_max_idx, debug=False):
#     small_row = triangle[row_idx]
#     big_row = triangle[row_idx-1]
#     size = len(small_row)
#
...
#
#         # handle the current state
#         if state == CROSSED or (state == LEVEL and small_row[level_start_idx] > small_row[idx]):
#
#             if prev_state == LEVEL:
#                 if diff > 1:
#                     raise ValueError('TBD Level to unexpected diff' + str(diff))
...
#                 end_idx += 1
#
#             # if small_row[idx] == big_row[idx] and big_row[idx] == big_row[idx+1]:
#             #     end_idx = idx
#             # else:
#             #     end_idx = idx + 1
#             #
#             # while end_idx < col_max_idx and (small_row[end_idx] > big_row[end_idx] or (small_row[end_idx] == big_row[end_idx] and small_row[end_idx] < small_row[end_idx-1])):
#             #     end_idx += 1
#
...
#
#             ######### EXCISE START
#             # fix if necessary? need to make a drop step
#             if big_row[end_idx-1] == big_row[end_idx] + 1:
#                 start_idx = end_idx - 1
#
#                 while start_idx > 0 and big_row[start_idx-1] == big_row[start_idx]:
#                     start_idx = start_idx - 1
#
#                 # hack: handling zero in small row?
#                 if small_row[end_idx-1] == 0:
#                     d = 0
#                 else:
#                     d = 0
#
#                 for k in range(start_idx, end_idx):
#                     small_row[k] = small_row[k] - d
#                     big_row[k] = big_row[k] + d
#
#             ######## EXCISE END
#
#             state = ABOVE
...
#             level_start_idx = size
#
#             # if end_idx == col_max_idx and big_row[end_idx] == big_row[end_idx+1] + 1:
#             #     # need a drop step
#             #
#
#             for k in range(level_start_idx, idx+1):
...
##########
### This was the "coda attempt" to make the drop. But it was flawed.
# def untangle_v3(triangle, row_idx, col_max_idx, debug=False):
#     small_row = triangle[row_idx]
#     big_row = triangle[row_idx-1]
#     size = len(small_row)
#
...
#
#
#             # CODA NEEDED when coda_height > 1
#
#
#             # if coda_start_idx < col_max_idx and small_row[coda_start_idx] > big_row[coda_start_idx]:
#             #     # gotta raise the upper path
#             #     for k in range(coda_start_idx, coda_end_idx):
#             #         print('just using coda diff')
#             #         coda_delta = small_row[k]  - big_row[k]
#             #         if coda_delta == 0:
#             #             coda_delta = 1
#             #         small_row[k] = small_row[k] - coda_delta - 1
#             #         big_row[k] = big_row[k] + coda_delta + 1
#
#                 # hack? is there a better way  to find coda_end_idx?
#                 # while coda_end_idx < col_max_idx and small_row[coda_end_idx-1] < small_row[coda_end_idx]:
#                 #     d = small_row[coda_end_idx] - small_row[coda_end_idx -1]
#                 #     small_row[coda_end_idx] = small_row[coda_end_idx] - d
#                 #     big_row[coda_end_idx] = big_row[coda_end_idx] + d
#                 #     coda_end_idx += 1
#
#             else:
...
#
#             if coda_start_idx < size:
#                 if big_row[coda_start_idx] > coda_height:
#                     print('NEED TO FIX THIS')
#
#             ######### EXCISE START
#             # fix if necessary? need to make a drop step
#             # if big_row[post_end_idx-1] == big_row[post_end_idx] + 1:
#             #     start_idx = post_end_idx - 1
#             #
#             #     while start_idx > 0 and big_row[start_idx-1] == big_row[start_idx]:
#             #         start_idx = start_idx - 1
#             #
#             #     # hack: handling zero in small row?
#             #     if small_row[post_end_idx-1] == 0:
#             #         d = 0
#             #     else:
#             #         d = 0
#             #
#             #     for k in range(start_idx, post_end_idx):
#             #         small_row[k] = small_row[k] - d
#             #         big_row[k] = big_row[k] + d
#
#             ######## EXCISE END
            ...
            if need_coda:
                ...
                if coda_end_idx < size and coda_end_height < small_row[coda_end_idx]:
                    ...
                    for i in range(coda_end_idx, extra_idx):
                        diff = small_row[i] - coda_end_height
                        small_row[i] = coda_end_height
                        big_row[i] = big_row[i] + coda_end_height
